...
function hasApiKey(provider: Provider) {
  ...
  if (provider.id === 'ollama' || provider.id === 'lmstudio' || provider.id === 'localLargeModel') return true
...
export function checkApiProvider(provider: Provider): void {
  ...
  if (provider.id !== 'ollama' && provider.id !== 'lmstudio' && provider.id !== 'localLargeModel') {
    if (!provider.apiKey) {
